...
CREATE POLICY "Public read count" ON registrations FOR SELECT USING (true);
...
-- ============================================================
-- TABLE: cities (with pricing fields for CRUD admin)
-- ============================================================
...
-- Public can read cities (landing page needs to show schedule)
CREATE POLICY "Public read cities" ON cities FOR SELECT USING (true);

-- Admin writes via service_role (bypass RLS) — no policy needed for service_role.
-- But for completeness, allow authenticated users to write (in case we add
-- a non-service-role admin path in the future):
...
CREATE INDEX IF NOT EXISTS idx_cities_date ON cities(date);
...
-- ============================================================
-- SEED DATA: 20 cities (run once, safe to re-run with ON CONFLICT)
-- ============================================================
...
ON CONFLICT (id) DO UPDATE SET
  ...
  vip_early_bird_price = EXCLUDED.vip_early_bird_price,
  early_bird_active = EXCLUDED.early_bird_active,
